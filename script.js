...
function updateStats() {
    const imageCards = document.querySelectorAll('.image-card:not(.usage-guide)');
    const comparisonCards = document.querySelectorAll('.comparison-card');
    const totalImages = imageCards.length + comparisonCards.length;
    const totalPrompts = totalImages; // 每张图片对应一个提示词
...
// 打开对比模态框
function openComparisonModal(button) {
    const card = button.closest('.comparison-card');
    const processedImage = card.querySelector('.processed .comparison-image');
    const title = card.querySelector('.comparison-title').textContent;
    const fullPrompt = card.querySelector('.full-prompt').textContent;
    
    // 设置模态框内容
    document.getElementById('modalImage').src = processedImage.src;
    document.getElementById('modalImage').alt = processedImage.alt;
    document.getElementById('modalTitle').textContent = title;
    document.getElementById('modalPrompt').textContent = fullPrompt;
